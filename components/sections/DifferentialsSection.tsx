          ...
          {content.items.map((item, index) => (
            <Reveal key={item} delay={index * 0.05}>
              <article className="group rounded-2xl border border-[color:var(--gold-200)] bg-white/90 px-6 py-5 shadow-[0_35px_70px_-62px_rgba(36,28,10,0.55)] transition-all duration-200 hover:-translate-y-0.5">
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[color:var(--gold-500)]">
                  0{index + 1}
                </p>
                <h3 className="mt-2 text-base font-semibold leading-relaxed text-[color:var(--gold-900)]">{item}</h3>
              </article>
            </Reveal>
          ))}
        </div>
